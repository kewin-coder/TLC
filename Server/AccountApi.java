 ...
 * Sessions are in-memory and disappear on restart. Owner access uses a server
 * environment secret. Deploy only behind HTTPS and configure a strong
 * TLC_ADMIN_SECRET privately before public deployment.
 */
public final class AccountApi {
    ...
    private static final class Session {
        ...
        final long expires;
        Session(String username, long expires) {
    ...
    private void register(HttpExchange e) throws IOException {
        ...
        if (username == null || password == null) {
            send(e, 400, "{\"error\":\"username and password required\"}");
            return;
        }
        char[] secret = password.toCharArray();
        try {
            if (!accounts.register(username, secret)) {
                send(e, 409, "{\"error\":\"Username unavailable\"}");
                return;
            }
            access.addPending(username);
            send(e, 201, "{\"status\":\"PENDING\"}");
        } catch (IllegalArgumentException ex) {
            send(e, 400, "{\"error\":\"Invalid registration details\"}");
        } catch (Exception ex) {
            send(e, 500, "{\"error\":\"Account creation failed\"}");
        } finally {
    ...
    private void login(HttpExchange e) throws IOException {
        ...
        String password = fields.get("password");
        if (username == null || password == null) {
            send(e, 400, "{\"error\":\"Credentials required\"}");
            return;
        }
        char[] passwordChars = password.toCharArray();
        try {
            if (!accounts.verify(username, passwordChars)) {
                send(e, 401, "{\"error\":\"Invalid credentials\"}");
                return;
            }
            AccountAccess.Status status = access.getStatus(username);
            if (status != AccountAccess.Status.APPROVED) {
                send(e, 403, "{\"status\":\"AWAITING_SYSADMIN_APPROVAL\"}");
                return;
            }
            ...
            e.getResponseHeaders().add("Set-Cookie",
                "TLC_SESSION=" + token + "; Path=/; HttpOnly; SameSite=Strict; Max-Age=43200");
            send(e, 200, "{\"ok\":true,\"username\":\"" + escape(username) + "\"}");
        } catch (Exception ex) {
            send(e, 500, "{\"error\":\"Login unavailable\"}");
        } finally {
    ...
    private void logout(HttpExchange e) throws IOException {
        ...
        e.getResponseHeaders().add("Set-Cookie",
            "TLC_SESSION=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0");
        send(e, 200, "{\"ok\":true}");
    }
    ...
    private void me(HttpExchange e) throws IOException {
        ...
        if (username == null) {
            send(e, 401, "{\"error\":\"Login required\"}");
            return;
        }
        try {
            access.requireApproved(username);
            send(e, 200, "{\"username\":\"" + escape(username) + "\",\"status\":\"APPROVED\"}");
        } catch (Exception ex) {
            sessions.remove(cookie(e, "TLC_SESSION"));
            send(e, 403, "{\"error\":\"Access revoked\"}");
        }
    }
    ...
    private void pending(HttpExchange e) throws IOException {
        if (!method(e, "GET")) return;
        if (!owner(e)) {
            send(e, 403, "{\"error\":\"Owner access required\"}");
            return;
        }
        try {
            StringBuilder body = new StringBuilder("{\"pending\":[");
            int count = 0;
            for (String username : access.listPending()) {
                if (count++ > 0) body.append(',');
                body.append('"').append(escape(username)).append('"');
            }
            body.append("]}");
            send(e, 200, body.toString());
        } catch (SQLException ex) {
            send(e, 500, "{\"error\":\"Queue unavailable\"}");
        }
    }
    ...
    private void setStatus(HttpExchange e) throws IOException {
        ...
        try {
            String username = fields.get("username");
            String value = fields.get("status");
            ...
            send(e, 400, "{\"error\":\"Invalid account or status\"}");
        } catch (SQLException ex) {
            send(e, 500, "{\"error\":\"Status update failed\"}");
        }
    }
    ...
    private static String cookie(HttpExchange e, String name) {
        String header = e.getRequestHeaders().getFirst("Cookie");
    ...
    private static boolean method(HttpExchange e, String expected) throws IOException {
        ...
        send(e, 405, "{\"error\":\"Method not allowed\"}");
        return false;
    }

    private static String readBody(HttpExchange e) throws IOException {
        byte[] bytes = e.getRequestBody().readNBytes(4097);
        if (bytes.length > 4096) throw new IOException("Request too large");
    ...
    private static void send(HttpExchange e, int code, String body) throws IOException {
        ...
        try (java.io.OutputStream output = e.getResponseBody()) {
            output.write(bytes);
        }
    }
}
