...
$dbFile = Join-Path $dataDir "tlc.db"
$keyFile = Join-Path $dataDir "encryption.key"
$backupFile = Join-Path ([Environment]::GetFolderPath("MyDocuments")) "TLC_Backup.tlcb"
function Has-ExistingData { return (Test-Path $dbFile) -or (Test-Path $keyFile) }
function Show-BackupGate {
    Write-Host ""
    Write-Host "============================================" -ForegroundColor Cyan
    Write-Host " TLC UPDATE / INSTALLATION" -ForegroundColor Cyan
    Write-Host "============================================" -ForegroundColor Cyan
    ...
    if ($choice -eq "1") {
        ...
        if (-not (Test-Path $backupTool)) { throw "TLC backup tool is missing from this installer package." }
        New-Item -ItemType Directory -Force -Path (Split-Path $backupFile) | Out-Null
        & java -jar $backupTool backup $backupFile
        if ($LASTEXITCODE -ne 0 -or -not (Test-Path $backupFile)) { throw "Backup creation failed. TLC was not updated." }
        Write-Host "Backup created: $backupFile" -ForegroundColor Green
    } else {
        $existing = Read-Host "Enter the full path to your TLC_Backup.tlcb"
        if (-not (Test-Path $existing)) { throw "The selected backup was not found. TLC was not updated." }
...
if (-not (Test-Path $sourceRoot)) { throw "TLC release payload directory is missing." }
...
Write-Host "Existing encryption key and database were preserved."
